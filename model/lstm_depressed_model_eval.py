...
class LSTMDepressedPointPredict(nn.Module):
    ...
    def loss_fuc(self, prediction, projection, target, predicted_time, time_to_predict):
        ...
        print(f"dist_loss :{dist_loss}\t angle_loss :{angle_loss}\t fit_loss:f{fit_loss}\t time_loss :{time_loss} \t ")
        # print(f"dist_loss_suppressed:{dist_loss_suppressed} \t angle_loss_suppressed:{angle_loss_suppressed} \t time_loss_suppressed:{time_loss_suppressed}")
    ...
    def cal_angle_loss(pred, target):
        ...
        raw_cosine = -nn.functional.cosine_similarity(pred, target, -1)
